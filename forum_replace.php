...
function convertlinebreaks ($message) {
    return preg_replace ("/\015\012|\015|\012/", "<br>", $message);
}


// Alles bis auf Neuezeile-Zeichen entfernen
function bbcode_stripcontents ($message) {
    return preg_replace ("/[^\n]", '', $message);
}
...
if($widthimg > 512) {  return 'Bilddatei zu groﬂ!! (max. 500px Breite erlaubt)';   } 
...
$new_message = $bbcode->parse ($message);
